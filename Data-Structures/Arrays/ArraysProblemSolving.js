...
function sepStr(str) {
  ...
  let maxIndex = getTheLargestLength(seperatedStr);
  let ew = "";
...
// sepStr("omar dsa dasdqw5 eqw sda");
function getTheLargestLength(arr) {
  let maxLength = 0;
  let index = 0;
  for (let i = 0; i < arr.length; i++) {
...
// function getTheMaxAndLow(height) {
//   while (height[0] == 0 || height[height.length - 1] == 0) {
//     if (height[0] == 0) {
//       height.shift();
//     }
...
//     if (height[b] == undefined) break;
//     if (height[b] > height[a]) {
//       // console.log("HERE");
//       // console.log(height[b], height[a]);
//       // console.log("HERE***********");
//       indecies[index++] = [a, b, height[b] - height[a]];
//       a = b;
//       b++;
...
// getTheMaxAndLow([4, 2, 0, 3, 2, 5]);
// // getTheMaxAndLow([0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1]);
...
//   let counter = 0;
//   let thePlaces = getTheMaxAndLow(height);
//   for (let key in thePlaces) {
//     let firstIndex = thePlaces[key][0];
//     let secIndex = thePlaces[key][1];
//     counter += loop(height.slice(firstIndex, secIndex + 1), key[0]);
//   }
//   return counter;
// };

// function loop(arr, target) {
//   let counter = 0;
//   for (let i = 0; i < arr.length; i++) {
//     if (arr[i] <= target) {
//       counter += arr[i];
...
var trap = function (height) {
  ...
  for (let i = 0; i < height.length; i++) {
    ...
    if (Math.abs(Math.min(maxLeft, maxRight) - height[i]) > 0) {
      maxWater += Math.abs(Math.min(maxLeft, maxRight) - height[i]);
    }
  }
  return maxWater;
};
...
var longestPalindrome = function (s) {
  ...
  while (p < s.length) {
    ...
    for (let i = p + 1; i < s.length; i++) {
      if (isPlaindrome(stored) && stored.length > longest.length) {
        longest = stored;
      }
      stored += s[i];
    }
